...
def main(page: ft.Page):
    ...
    # We’ll keep the general EAP options, but multiple sub-options for inner methods
    ...
    def build_test_config_view(test_id: str):
        ...
        and additional parameters such as server IP, port, timeout, certificate file
        for EAP-TLS, and "inner method" for PEAP/EAP-TTLS/EAP-FAST."""
        ...
        # INNER METHOD DROPDOWN (for PEAP/EAP-TTLS/EAP-FAST only)
        inner_method_dropdown = ft.Dropdown(
            label="Inner Method",
            width=300,
            value=config["inner_method"],  # e.g. "MSCHAPv2"
        )

        # EAP-TTLS: "anonymous identity" field
        anonymous_identity_tf = ft.TextField(
            label="Anonymous Identity (TTLS)",
            width=300,
            value=config["anonymous_identity"]
        )

        # EAP-FAST: "fast provisioning" switch
        fast_provision_switch = ft.Switch(
            label="Allow FAST Provisioning",
            value=config["fast_provisioning"]
        )

        # FILE PICKER for EAP-TLS
        certificate_picker = ft.FilePicker(on_result=lambda e: handle_cert_selected(e))
        page.overlay.append(certificate_picker)  # Add to page overlay so we can use it
        ...
        cert_container = ft.Column(
            ...
            visible=(config["eap_type"] == "EAP-TLS")  # Shown only for TLS
        )
        ...
        # Container for "inner method" + optional fields
        # Shown for EAP types that require an inner method
        inner_method_container = ft.Column(
            controls=[],
            visible=False
        )

        # We'll dynamically configure it in the update_eap_ui_visibility() function

        def update_eap_ui_visibility():
            """Show/hide the correct UI elements depending on EAP type."""
            current_eap = eap_dropdown.value

            # EAP-TLS => show certificate container
            cert_container.visible = (current_eap == "EAP-TLS")

            # If it's one of the others, show the "inner_method_container"
            if current_eap in ["PEAP", "EAP-TTLS", "EAP-FAST"]:
                # Populate possible inner methods
                methods = inner_methods_map[current_eap]  # e.g. ["MSCHAPv2", "GTC"]
                inner_method_dropdown.options = [ft.dropdown.Option(m) for m in methods]

                # If the current stored inner_method is not in that list, reset
                if config["inner_method"] not in methods:
                    config["inner_method"] = methods[0]  # pick the first
                    inner_method_dropdown.value = methods[0]
                else:
                    # We keep the saved one
                    inner_method_dropdown.value = config["inner_method"]

                # Show container
                inner_method_container.visible = True

                # Show/hide the TTLS / FAST extras
                # EAP-TTLS => show anonymous identity
                anonymous_identity_tf.visible = (current_eap == "EAP-TTLS")
                # EAP-FAST => show fast provisioning
                fast_provision_switch.visible = (current_eap == "EAP-FAST")

            else:
                # Not PEAP/TTLS/FAST => hide
                inner_method_container.visible = False

            page.update()

        # Build the "inner_method_container" with child controls
        ...
        def save_and_run_test(e):
            ...
            # If we have an inner method container visible:
        ...
        view = ft.View(
            ...
            [
                ...
                # Inner method container (only for PEAP/TTLS/FAST)
                inner_method_container,

                # Certificate UI (only for EAP-TLS)
                cert_container,
        ...
        # Initialize UI visibility based on current config
        update_eap_ui_visibility()

        return view

    # ─────────────────────────────────────────────────────────
    # 4. SETTINGS VIEW (FOR THEME TOGGLE, ETC.)
    # ─────────────────────────────────────────────────────────
