...
def run_peap_test(username, password, server_ip, port, timeout, inner_method):
    ...
    # Simulate a success or failure result here:
    # e.g., return True/False, or an error message
...
def main(page: ft.Page):
    ...
    # ------------------------------------------------------------------
    # CLIENTS DATA (Renamed & Enhanced for RadSec, Cert, Secret)
    # ------------------------------------------------------------------
    clients = [
        {
    ...
    def build_login_view():
        username_tf = ft.TextField(
            label="Username", 
            width=300,
            on_submit=lambda e: login_click(e)  # triggers login on Enter
            )
    ...
    def build_test_config_view(test_id: str):
        ...
        def save_and_run_test(e):
            ...
            if config.get('eap_type'):
                ...
                print(f"Running test with EAP type: {eap_type}")
                if eap_type == 'PEAP':
                    print('Sending PEAP to test function...')
                    ...
                    print(success)
                elif eap_type == 'EAP-TLS':
                    print('Sending EAP-TLS to test function...')
                elif eap_type == 'EAP-TTLS':
                    print('Sending EAP-TTLS to test function...')
                elif eap_type == 'EAP-FAST':
                    print('Sending EAP-FAST to test function...')
    ...
    def build_clients_view():
        def update_client_list():
            """Rebuild the UI controls for each client."""
        ...
        def open_client_cert_picker(index):
            # store index in the picker's user data
