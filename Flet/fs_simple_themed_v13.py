...
def main(page: ft.Page):
    ...
    # DIAGNOSTICS DATA
    # ------------------------------------------------------------------
    # This might hold CPU/memory data for each host that you fetch via SSH.
    # For demonstration, we'll store random or static usage data.
    # We'll visualize these in the /diagnostics view.
    diagnostics_data = {
        "UbuntuTestVM": {
            "cpu_percent": [20, 25, 23, 30],      # sample data points
    ...
    def build_tests_view():
        ...
        instructions = ft.Text("Select a test to configure and run, or configure clients / VMs / diagnostics.", size=16)
    ...
    def build_test_config_view(test_id: str):
        # [Omitted here for brevity—unchanged from your version]
        # ...
        # (See your existing code for test config)
        pass  # We'll paste your existing test config code below

    # Just paste your existing 'build_test_config_view' here:
    def build_test_config_view(test_id: str):
    ...
    # Paste your existing build_clients_view() here:
    def build_clients_view():
    ...
    # Paste your existing build_vms_view() here:
    def build_vms_view():
        def update_vms_list():
    ...
    def build_diagnostics_view():
        # We'll show CPU usage data in a line chart for each host,
        # stored in `diagnostics_data`.
        # In a real app, you'd fetch updated data via SSH, then rebuild the chart.

        chart_container = ft.Column()

        # We create a function to rebuild the chart and refresh the UI
        def update_charts():
            chart_container.controls.clear()

            # For each host, create a line chart showing CPU usage over time
        ...
        def create_line_chart_for_host(hostname, data_dict):
            # data_dict might contain "cpu_percent", "memory_percent", "timestamps", etc.
            df = {
        ...
        def fetch_data_click(e):
            # This is where you'd SSH to each host, collect metrics,
            # transform them, and store them in `diagnostics_data`.
            # For now, we just simulate new random data or increments:
            import random
            ...
            for host, d in diagnostics_data.items():
                ...
                # Generate random CPU/mem usage or increment
        ...
        # We'll build the diagnostics view with a refresh button
        ...
        view = ft.View(
            ...
            [
                ...
                chart_container  # We'll place Plotly charts here
            ],
            vertical_alignment=ft.MainAxisAlignment.START,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            scroll=ft.ScrollMode.AUTO
        ...
        # On first entry, build the initial charts
        update_charts()
        return view
...
if __name__ == "__main__":
    # Make sure you have `pip install plotly` for this to work
    ft.app(target=main)
