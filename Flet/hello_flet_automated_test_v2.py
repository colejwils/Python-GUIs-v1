...
def main(page: ft.Page):
    ...
    page.theme = ft.Theme(
        color_scheme=ft.ColorScheme(
            primary=ft.Colors.from_hex("#00ADEF"),    # Forescout teal
            on_primary=ft.Colors.WHITE,
            secondary=ft.Colors.from_hex("#74C236"),  # Forescout green (CTA)
        ...
        def login_click(e):
            if username_tf.value.strip() and password_tf.value.strip():
                ...
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("Please enter username and password!", color=ft.Colors.WHITE),
                    bgcolor=ft.Colors.from_hex("#F36F21")  # an orange accent, if desired
                )
            ...
            controls=[
                ...
                ft.ElevatedButton(
                    ...
                    style=ft.ButtonStyle(
                        ...
                        bgcolor={ft.MaterialState.DEFAULT: ft.theme.color_scheme.secondary},  
                    ),
                ),
            ],
        )
    ...
    def build_test_config_view(test_id: str):
        ...
        def save_and_run_test(e):
            ...
            page.snack_bar = ft.SnackBar(
                content=ft.Text(f"Running {test_title} with EAP={config['eap_type']}...", color=ft.Colors.WHITE),
                bgcolor=ft.theme.color_scheme.primary
            )
            page.snack_bar.open = True
            page.update()
