...
def main(page: ft.Page):
    ...
    # ------------------------------------------------------------------
    # REMOTE LINUX HOSTS (FOR RESOURCE MONITORING)
    # ------------------------------------------------------------------
    linux_hosts = [
        {
            "host_name": "LinuxHostA",
    ...
    def build_linux_hosts_view():
        ...
        return ft.View(
            ...
            [
                ft.Row(
                    [
                        ft.Text("Configure Linux Hosts (Resource Monitoring)", size=24, weight=ft.FontWeight.W_600),
                        ft.IconButton(icon=ft.Icons.ARROW_BACK, tooltip="Back to Main", on_click=go_back),
                    ],
                    alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                ),
                ft.Divider(),
                ft.Text("Add, edit, or remove Linux hosts below. Then 'Test' to verify SSH connectivity:", size=16),
                hosts_col,
                ft.Row(
                    [
